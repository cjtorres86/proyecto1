    ...
    this.sub = combineLatest([this.caseState.mesActivo$, this.caseState.slepActivo$, this.caseState.campoActivo$])
      .pipe(
        switchMap(([mes, slepId, campoId]) => {
          if (slepId) return this.caseState.getContenedorConValores(slepId).pipe(map((r) => ({ campos: r.campos, campoId })));
          if (mes) return this.caseState.getTotalGeneral(mes.mes, mes.anio).pipe(map((r) => ({ campos: r.campos, campoId })));
          return of(null);
        }),
      )
      .subscribe((resultado) => {
